-- Expand borough filter from LIC-only to full Queens.
-- Accepts both 'queens' and legacy 'lic' (treated as full Queens).

create extension if not exists postgis with schema extensions;

-- complexes_mvt_tile: return type unchanged (bytea), so CREATE OR REPLACE is fine.
...
-- complexes_in_bounds: must DROP before recreate (OUT row type cannot change in place).
do $$
declare
  r record;
begin
